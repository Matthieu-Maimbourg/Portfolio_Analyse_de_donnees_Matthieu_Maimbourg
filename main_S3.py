...
colonne_quantitatives = contenu.select_dtypes(include=["int64","float64"])

noms = list(colonne_quantitatives.columns)
moyennes = list(colonne_quantitatives.mean().round(2))
medianes = list(colonne_quantitatives.median().round(2))
modes = []
for col in noms:
    serie = colonne_quantitatives[col].dropna()
    mode_col = serie.mode()
    modes.append(round(float(mode_col.iloc[0]),2) if len(mode_col) > 0 else None)

ecarts_types = list(colonne_quantitatives.std().round(2))
ecarts_absolus_moy = []
for col in noms:
    serie = colonne_quantitatives[col].dropna()
    m = serie.mean()
...
iqr = list((colonne_quantitatives.quantile(0.75) - colonne_quantitatives.quantile(0.25)).round(2))
idr = list((colonne_quantitatives.quantile(0.90) - colonne_quantitatives.quantile(0.10)).round(2))
...
for col in noms :
    plt.figure()
    plt.boxplot(colonne_quantitatives[col].dropna(), vert=True)
